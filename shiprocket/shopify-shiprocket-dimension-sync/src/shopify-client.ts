...
export class ShopifyClient {
  ...
  /**
   * Clears the cached token from disk.
   */
  ...
  /**
   * Fetches or refreshes the access token using client-credentials authentication.
   */
  public async getAccessToken(): Promise<string> {
    ...
    // 2. Fetch new token from Shopify
    ...
    try {
      ...
      // Verify scope contains read_products
      const scopesStr = scope || '';
      const scopes = scopesStr.split(',').map((s: string) => s.trim());
      if (!scopes.includes('read_products')) {
        throw new Error(`Insufficient Shopify API scopes. Expected "read_products", got "${scopesStr}".`);
      }
      ...
      // Write file with 0600 permissions where supported
  ...
  /**
   * Executes a GraphQL query on Shopify, handling retries, 401s, 429s, and throttling.
   */
  private async executeGqlQuery(query: string, variables: any, retryCount = 0): Promise<any> {
    ...
    try {
      ...
      if (response.data.errors) {
        ...
        // Check if any error is due to throttling
      ...
      // Handle throttling dynamically using cost extensions
      ...
      if (cost && cost.throttleStatus) {
        ...
        // If available points are less than the requested cost (plus a small safety buffer), sleep to restore
      ...
      // Handle 401 Unauthorized (invalidated or expired token)
  ...
  /**
   * Query the shop identity (sanity read-only test).
   */
  ...
  /**
   * Fetches all variants from active Shopify products with pagination.
   */
  public async fetchActiveVariants(options: { includeArchivedAndDrafts?: boolean } = {}): Promise<ShopifyVariant[]> {
    ...
    while (hasNextPage) {
      ...
      for (const edge of edges) {
        ...
        // Ignore archived and draft products by default
    ...
    // Validate SKUs: blank & duplicates
    return this.validateAndFilterVariants(variants);
  }

  /**
   * Performs validation: rejects blank SKUs, fails on duplicate SKUs.
   */
  private validateAndFilterVariants(variants: ShopifyVariant[]): ShopifyVariant[] {
    ...
    for (const v of variants) {
      ...
      // Group by SKU to detect duplicates (case-sensitive)
    ...
    // Filter out blank variants from our return list
