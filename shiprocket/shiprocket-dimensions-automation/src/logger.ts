...
class Logger {
  ...
  private write(level: string, message: string) {
    ...
    // Scrub tokens, Bearer authorization, client secrets, and passwords
    ...
    scrubbed = scrubbed.replace(/"access_token"\s*:\s*"[^"]+"/gi, '"access_token": "[REDACTED]"');
    scrubbed = scrubbed.replace(/client_secret=[a-zA-Z0-9]+/g, 'client_secret=[REDACTED]');
